...
const steps = [
  ...
  },
  {
    ...
    text: "Добавьте аккаунт через cookies. Система проверит сессию и покажет, готов ли профиль к публикации. Для боевого режима лучше сразу подключать прокси.",
    action: "Рекомендуем использовать отдельный динамический мобильный прокси на каждый Threads-профиль. Так Threads видит привычную сетевую среду для аккаунта, а риск блокировки ниже. Без прокси аккаунт может быстрее попасть под проверки, особенно если запускать много профилей с одного сервера.",
  ...
  },
  {
    ...
    text: "ThreadsGo соберет тренды, подготовит черновики и поставит публикации в очередь. Вы можете редактировать каждый пост вручную.",
    action: "Внутри проекта нажмите «Запустить анализ трендов» или «Сгенерировать пост».",
