...
class User(Base, TimestampMixin):
    ...
    subscription_paid_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    subscription_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    complimentary_access_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    complimentary_access_plan: Mapped[str | None] = mapped_column(String(32), nullable=True)
    complimentary_access_reason: Mapped[str | None] = mapped_column(String(255), nullable=True)
    tribute_last_event_type: Mapped[str | None] = mapped_column(String(128), nullable=True)
    tribute_last_event_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
